...
import type { Document, DocumentFile } from '@/lib/api';
      ...
      <div className="relative w-full max-w-5xl max-h-[90vh] mx-4 bg-surface rounded-lg shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-start justify-between p-4 border-b border-border-light bg-lighter">
          <div className="flex-1 min-w-0 pr-4">
            <h2 className="font-heading text-xl font-bold text-charcoal truncate">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-md hover:bg-light transition-colors"
            aria-label="Close"
          >
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-4 text-sm text-steel">
              {doc.attributes.publishedAt && (
                <span>
                  Published: {formatDate(doc.attributes.publishedAt)}
                </span>
              )}
              {fileSize > 0 && (
              ...
              {doc.attributes.currentVersion?.data?.attributes?.versionNo && (
                <span>
                  Version {doc.attributes.currentVersion.data.attributes.versionNo}
                </span>
              )}
            </div>
