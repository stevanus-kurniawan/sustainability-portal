/**
 * API Client for SLMS Public Endpoints
 * Uses fetch with caching for server components
 */
...
async function fetchApi<T>(
  ...
  // Use Next.js revalidation instead of cache header to avoid conflicts
...
export async function getLibrary(params?: {
  ...
  return fetchApi<Document[]>(`/public/library${query ? `?${query}` : ''}`, {
    revalidate: 60, // 1 minute for library
    tags: ['library'],
  });
}
