...
export async function GET(
  ...
  try {
    const res = await fetch(`${getInternalApiBase(request)}/admin/categories/${categoryId}/sub-contents/${subId}`, {
...
export async function DELETE(
  ...
  try {
    ...
    if (res.status === 204 || res.ok) return new NextResponse(null, { status: res.status });
    const data = await res.json().catch(() => ({}));
    return NextResponse.json(data || { message: res.statusText }, { status: res.status });
  } catch (err) {
    return NextResponse.json(
